...
function afficherLaReponse(retour){
    let elt = document.getElementById("product-result");
    for (let i = 0; i < retour.length; i++) {
        creationEnfant(retour[i],elt);
        console.log(retour[i].colors);
...
setTimeout(() => {
    ...
    for (let i = 0; i < clickPage1.length; i++) {
        clickPage1[i].addEventListener('click', function(event) {
            ...
            window.location.assign("produit.html?"+ click_id);
